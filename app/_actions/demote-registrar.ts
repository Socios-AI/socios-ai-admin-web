...
export type DemoteRegistrarResult =
  | { ok: true }
  | { ok: false; error: "FORBIDDEN" | "VALIDATION" | "API_ERROR"; message?: string };

export async function demoteRegistrarAction(input: {
  userId: string;
  reason: string;
}): Promise<DemoteRegistrarResult> {
  ...
  const { error } = await sb.rpc("demote_registrar", { p_user_id: userId, p_reason: reason });
